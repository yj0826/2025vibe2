...
def load_rankings():
    if os.path.exists(RANKING_FILE):
        with open(RANKING_FILE, "r", newline="", encoding="utf-8") as f:
            reader = csv.reader(f)
            return [(name, int(tries)) for name, tries in reader]
    return []
...
if not st.session_state.game_over:
    ...
    if st.button("🔍 제출하기"):
        ...
        if guess == answer:
            ...
            st.session_state.pending_rank = st.session_state.tries
        elif guess < answer:
...
if st.session_state.get("pending_rank"):
    ...
    if st.button("🏅 랭킹에 등록하기"):
        if name.strip():
            ...
            # 랭킹 정렬 후 상위 5명 유지
            st.session_state.rankings = sorted(st.session_state.rankings, key=lambda x: x[1])[:5]
